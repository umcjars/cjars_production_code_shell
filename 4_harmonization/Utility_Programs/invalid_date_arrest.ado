...
Invalid Arrest Date
===================
Utility program for logical validation of arrest date variables. Dates set 
...
==============================================================================*/
#delimit;
program define invalid_date_arrest;
	local data="`1'";
	...
	foreach stub in arr_arr arr_book {;
		...
		if r(N)>0 {;
			...
			if r(N)>0{;
				if `j'==1{;
					file open myfile using "${codedir}\4_harmonization\1_prepared\\`data'\\invalid_date_arr.txt", write replace;
					file write myfile "variable,count_invalid"_n;
					file close myfile;
				};
				file open myfile using "${codedir}\4_harmonization\1_prepared\\`data'\\invalid_date_arr.txt", write append;
